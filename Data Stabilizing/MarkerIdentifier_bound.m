...
function [prev_row, org_row] = organiseRow(abs_row, prev_row, row)
    ...
    dist = nan(parts, parts);
    for part = 1:parts
        if isnan(org_row(part, :))
            for temp_part = 1:parts
                cols = dimens*(part-1)+1:dimens*(part-1)+3;
                dist(part, temp_part) = norm(row(temp_part, :) - prev_row(end, cols));
            end
        end
    end
    ...
    %update previous row
    prev_row(abs_row+1, :) = prev_row(abs_row, :);
    for part = 1:parts
        cols = dimens*(part-1)+1:dimens*part;
        if norm(prev_row(abs_row, cols) - org_row(part, :)) < tolerance(2)
            prev_row(abs_row+1, cols) = org_row(part, :);
        end
        if norm(prev_row(abs_row, cols) - org_row(part, :)) > tolerance(3)
